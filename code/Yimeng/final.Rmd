...
ratio_return$Year.adj.adj<- NULL
...
```{r}
#predict 2014
train2014 <- train[train$Fiscal.Year < 2014,]
test2014 <- train[train$Fiscal.Year == 2014,]

model2014 <- glm(logreturn ~ Book.to.Market.Ratio.log + Sales.Per.Share.log + Return.On.Equity.log + Operating.Profit.Margin.log + Receivable.Turnover.log + Total.Debt.Ratio.log+Current.Ratio.log, data=train2014)

distPred2014 <- predict(model2014, test2014)  # predict distance
actuals_preds2014 <- data.frame(cbind(test2014[c(1:5,14)], actuals=test2014$actual_re, predicteds=distPred2014)) 


actuals_preds2014[order(-actuals_preds2014$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#good: 0.3006662		
actuals_preds2014[order(actuals_preds2014$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#bad: 0.05608732		
```

```{r}
#predict 2015
train2015 <- train[train$Fiscal.Year < 2015 & train$Fiscal.Year >= 2010,]
test2015 <- train[train$Fiscal.Year == 2015,]

model2015 <- glm(logreturn ~ Book.to.Market.Ratio.log + Sales.Per.Share.log + Return.On.Equity.log + Operating.Profit.Margin.log + Receivable.Turnover.log + Total.Debt.Ratio.log+Current.Ratio.log, data=train2015)

distPred2015 <- predict(model2015, test2015)  # predict distance
actuals_preds2015 <- data.frame(cbind(test2015[c(1:5,14)], actuals=test2015$actual_re, predicteds=distPred2015)) 


actuals_preds2015[order(-actuals_preds2015$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#good: 0.02671635				
actuals_preds2015[order(actuals_preds2015$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#bad: -0.02587445				
```
 
```{r}
#predict 2016
train2016 <- train[train$Fiscal.Year < 2016 & train$Fiscal.Year >= 2010,]
...
actuals_preds2016[order(-actuals_preds2016$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#good: 0.350472				
actuals_preds2016[order(actuals_preds2016$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#bad: 0.2266411					
```
...
actuals_preds2017[order(-actuals_preds2017$predicteds),][1:5,] %>% summarise(totreturn = mean(actuals))#good: 0.03060938					
actuals_preds2017[order(actuals_preds2017$predicteds),][1:5,] %>% summarise(totreturn = mean(actuals))#bad: 0.06896462	
...
actuals_preds2018 <- data.frame(cbind(test2018[c(1:5,14)], actuals=test2018$logreturn, predicteds=distPred2018)) 


actuals_preds2018[order(-actuals_preds2018$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#good: 0.1329313						
actuals_preds2018[order(actuals_preds2018$predicteds),][1:5,] %>% summarise(totreturn = mean(actual_re))#bad: 0.2015754		
