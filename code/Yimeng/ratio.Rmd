...
#start calculation
ratio <- ratio %>% mutate(Interest.Coverage.Ratio = ifelse((Interest.Expense..Net==0) | Interest.Expense..Net >0, NA, round(Operating.Income..Loss./(-1*Interest.Expense..Net),6)))
mean <- mean(ratio$Interest.Coverage.Ratio, na.rm=T) #50.039
ratio$Interest.Coverage.Ratio[ratio$Interest.Expense..Net>0] <- mean
...
sum(ratio$Depreciation...Amortization > 0, na.rm = T)#13971
#Negative depreciation, on the other hand, accounts for the opposite process of an asset gaining value over time. There is nothing wrong
...
ratio$Cash.Coverage.Ratio[ratio$Interest.Expense..Net>0] <- mean
...
sum(is.na(ratio$Return.On.Capital)) #6
sum(ratio$Return.On.Capital <0, na.rm=T) #1255
```
...
ratio <- mutate(ratio, ROE.bad.or.good = ifelse((After.Tax.Operating.Income <0) &(Total.Equity >0), 'bad', ifelse(!is.na(Return.On.Equity),'good', NA)))
sum(is.na(ratio$Return.On.Equity)) #15
sum(ratio$Return.On.Equity <0, na.rm=T) #1649
sum(ratio$ROE.bad.or.good =='bad',na.rm=T) #1080
sum(is.na(ratio$ROE.bad.or.good))#15, consistent
```
...
sum(is.na(ratio$Market.to.Book.Ratio)) #781
...
Book to Market Ratio: (Total Equity/Shares)/ Stoke Price
```{r}
summary(ratio$Close)
summary(ratio$Total.Equity)
summary(ratio$Shares..Diluted.)
...
```{r}
names(ratio)
final <- ratio[,c(1:8,37:46, 48:53, 55, 57, 62:66)]
final_withoutmissing <- na.omit(final)
dim(final)#[1] 13992    31
dim(final_withoutmissing) #11718 31
#we have 2297 rows with no missing values in technology sector 
nrow(final_withoutmissing[final_withoutmissing$Sector == 'Technology',]) # 2297
```
