...
tech.adj <- subset(df.adj, Sector == 'Technology')
dim(tech.adj)# 2089   32
```
...
After removing observation smaller than -1, the distribution is slightly left skewed and the correlation with stock return is low
```{r}
cor(tech$Operating.Profit.Margin, tech$Return) #-0.01945389
ggplot(tech[tech$Operating.Profit.Margin > -1,]) + geom_histogram(aes(x = Operating.Profit.Margin))
```

Let's try transform the data, first try cube transformation, doesn't work
```{r}
...
ggplot(tech) + geom_histogram(aes(x = Operating.Profit.Margin.cube)) #doesn't work
```

log transformation, doesn't improve the skewness either 
```{r}
...
```{r}
sd <- sd(tech$Operating.Profit.Margin) #0.4778129
mean <- mean(tech$Operating.Profit.Margin) # 0.04479941
ci <- c(mean - 3*sd, mean + 3*sd) #-1.388639  1.478238
